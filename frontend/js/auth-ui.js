...
function initAuthUI() {
    ...
    if (user && token) {
        ...
        userAccountContainer.innerHTML = `
            <div class="relative group">
                <button class="flex items-center gap-2 text-black hover:text-red-600 transition cursor-pointer">
                    ...
                    <span class="text-lg font-semibold text-orange-600">${user.ten_dang_nhap}</span>
                ...
                <div class="absolute right-0 top-full mt-2 w-64 bg-white rounded-xl shadow-2xl border border-gray-200 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                    <div class="p-4 border-b border-gray-100">
                        <p class="font-bold text-gray-800 text-lg">${user.ten_dang_nhap}</p>
                        <p class="text-gray-500 text-sm">${user.email}</p>
                    </div>
                    <div class="py-2">
                        <a href="${getBasePath()}pages/account.html" class="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition">
                            <svg class="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
                            </svg>
                            <span class="text-gray-700 font-medium">Tài khoản của tôi</span>
                        </a>
                        <a href="${getBasePath()}pages/orders.html" class="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition">
                            <svg class="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        userAccountContainer.innerHTML = `
            <a href="${getBasePath()}pages/login.html" class="flex items-center gap-2 text-black hover:text-red-600 transition">
